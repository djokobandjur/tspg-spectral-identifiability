...
The first archival release is intended to contain only the artifacts needed to reproduce or audit the reported results:
...
The manuscript itself is **not** part of the public repository or Zenodo reproducibility archive.

The public release will not contain:

- manuscript PDF files;
- LaTeX/source files for the manuscript;
- submission packages or cover letters;
- contribution-information sheets;
- reviewer/editor correspondence;
- internal manuscript-development or manuscript-governance artifacts.

The repository may refer to the study title, reported result labels (M1--M5), and manuscript sections solely to map claims to reproducibility evidence.
...
`reported result -> compact result artifact -> run/config -> code -> split/checkpoint manifest -> SHA-256 provenance`.

The GitHub release and corresponding Zenodo archive will be versioned together so that the archival DOI identifies an immutable reproducibility snapshot rather than a manuscript copy.
