...
- [ ] Stage the exact verified non-Git assets in the release-upload workspace and independently rehash every staged copy
...
The non-Git distribution boundary is fixed in `manifests/TSPG_RELEASE_ASSET_PLAN_v1_0_20260829.json`: GitHub Release receives the checkpoint plus five compact evidence ZIPs; Zenodo receives the same six convenience assets plus only the standalone large numerical binaries not already present inside those ZIPs. The standalone set has eight objects: AG1 gradients (387,317,888 bytes), H1-0010 geometry (776,283,320), H1-0011 complement basis (79,126,618), H1-0015 tail geometry (232,412,532), AG2 gradients (387,317,888), H1-0016 derived cross-fold binary (390,626,716), AP gradients (774,635,648), and H1-0019 fit-arm bases (154,928,108). All eight actual retained/transferred objects have now been checked against their locked SHA-256 identities with `8/8 PASS`; the public record is `audits/TSPG_STANDALONE_LARGE_ARTIFACT_SOURCE_BYTE_VERIFICATION_v1_0_20260829.md`. The next separate gate is to stage these exact bytes for release upload and rehash the staged copies.
