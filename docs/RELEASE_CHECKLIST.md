# Archival release checklist

The first GitHub/Zenodo release should not be tagged until all items below are complete.

- [ ] Public code set promoted from SHA-verified evidence artifacts
- [ ] Locked configurations promoted
- [ ] Dataset split/index manifest promoted
- [ ] Compact H1-0015--H1-0019 result artifacts promoted
- [ ] Public numerical/reproducibility audits promoted
- [ ] Large/restricted artifacts documented by exact SHA-256 and acquisition/reconstruction instructions
- [ ] Manuscript-to-artifact map completed with no `pending promotion` entries for reported M1--M5 results
- [ ] Clean-environment reproduction instructions tested
- [ ] Public release SHA-256 manifest generated
- [ ] Code/data licensing selected and documented
- [ ] `CITATION.cff` updated with release version and Zenodo DOI
- [ ] GitHub release tagged
- [ ] Zenodo archive minted from the same immutable GitHub release
- [ ] Data Availability and Code Availability statements updated with the persistent DOI
