...
- [ ] Transfer/materialize and independently rehash the standalone archival numerical binaries; H1-0011 complement metadata size is recovered, while the exact H1-0019 fit-arm-bases size still requires the actual bytes
...
The non-Git distribution boundary is fixed in `manifests/TSPG_RELEASE_ASSET_PLAN_v1_0_20260829.json`: GitHub Release receives the checkpoint plus five compact evidence ZIPs; Zenodo receives the same six convenience assets plus only the standalone large numerical binaries not already present inside those ZIPs. Recorded sizes are AG1 gradients (387,317,888 bytes), H1-0010 geometry (776,283,320), H1-0011 complement basis (79,126,618), H1-0015 tail geometry (232,412,532), AG2 gradients (387,317,888), H1-0016 derived cross-fold binary (390,626,716), and AP gradients (774,635,648). The H1-0011 size was recovered from the SHA-locked `raw_output` entry inside the authoritative H1-0011 runtime result and still requires transfer-time byte verification. Exact size remains intentionally unset only for the H1-0019 fit-arm bases until its actual bytes are transferred and rehashed.
