...
- [x] Standalone Zenodo staged-copy SHA-256 manifest received and checked: 8/8 hashes exactly match the locked artifact identities (`manifests/TSPG_ZENODO_STANDALONE_STAGING_SHA256_v1_0_20260829.txt`; `audits/TSPG_ZENODO_STANDALONE_STAGING_SHA_VERIFICATION_v1_0_20260829.md`)
- [ ] Retain/promote the generated standalone staging JSON metadata manifest, then stage and independently rehash the six convenience assets (checkpoint + five compact H1-0015--H1-0019 evidence ZIPs) to complete the non-Git release-upload workspace
...
The non-Git distribution boundary is fixed in `manifests/TSPG_RELEASE_ASSET_PLAN_v1_0_20260829.json`: GitHub Release receives the checkpoint plus five compact evidence ZIPs; Zenodo receives the same six convenience assets plus only the standalone large numerical binaries not already present inside those ZIPs. The standalone set has eight objects totaling 3,182,648,718 bytes. All eight retained/transferred source objects are exact size/SHA-256 matches, and the supplied staged-copy checksum manifest independently reproduces the same eight SHA-256 identities (`8/8 PASS`). Its exact public copy is `manifests/TSPG_ZENODO_STANDALONE_STAGING_SHA256_v1_0_20260829.txt`; the checksum file itself is 1,026 bytes with SHA-256 `c809c81e2f8be8a1d93bb473fdb837900cc15ef9592c4a6014ad3dae2fff2f3a`. The small generated staging JSON remains to be retained so the stage-directory and per-file status metadata are preserved explicitly.
