...
The non-Git release-upload workspace is now fully staged and byte-verified. The standalone Zenodo payload contains eight objects totaling 3,182,648,718 bytes with 8/8 staged-copy SHA-256 matches. The convenience payload contains the checkpoint plus five compact evidence ZIPs totaling 353,260,167 bytes with 6/6 staged-copy SHA-256 matches. The raw convenience staging JSON is 1,689 bytes with SHA-256 `a5be650c9b595c423819a74d32b4b7b53a6fbf811bf1faadafe8354cf7460176`; because it contains a host-local absolute path and username, the public repository retains a sanitized semantic copy at `manifests/TSPG_CONVENIENCE_ASSETS_STAGING_MANIFEST_PUBLIC_v1_0_20260829.json`. Upload to GitHub Release/Zenodo and post-upload verification remain separate pending gates.
