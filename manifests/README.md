...
- `CODE_SHA256.txt` — SHA-256 identities of the exact final non-superseded public analysis code copied from the locked execution starters, plus the explicit pending identity of the checkpoint model-definition source.
...
- `LARGE_ARTIFACTS_SHA256.csv` — identities, verified sizes where available, and distribution status of large artifacts that are not stored in ordinary Git history, including the H1-0011 M1 complement dependency.
