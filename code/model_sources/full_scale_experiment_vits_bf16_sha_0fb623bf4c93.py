...
"""
=============================================================================
Full-Scale Positional Encoding Analysis for Vision Transformers — ViT-S/16
=============================================================================
...
Model:      ViT-Small/16 (12 layers, 384 dim, 6 heads, patch 16x16 → 196 patches)
Seeds:      6 independent runs per configuration (42, 123, 456, 789, 1011, 1213)
PE types:   Sinusoidal, Learned, RoPE, ALiBi

Requirements:
    pip install torch torchvision timm matplotlib numpy scipy scikit-learn tqdm

Hardware:   1x NVIDIA H200 GPU per seed session 
...
class VisionTransformer(nn.Module):
    """ViT-S/16 with configurable positional encoding."""
...
def train_model(model, train_loader, val_loader, config, device, output_dir):
    ...
    scaler = torch.amp.GradScaler('cuda', enabled=False)  # Mixed precision
    ...
    print(f"\n[FINAL] Artifacts stored persistently in: {output_dir}")
...
def plot_training_curves(all_histories, output_dir):
    ...
    ax1.set_title('Accuracy during training \u2014 ImageNet-100 (ViT-S/16)')
    ax1.legend(); ax1.grid(True, alpha=0.3)

    ax2.set_xlabel('Epoch'); ax2.set_ylabel('Validation loss')
    ax2.set_title('Validation loss during training \u2014 ImageNet-100 (ViT-S/16)')
...
def plot_pca_tsne(all_pe_matrices, output_dir):
    ...
    fig.suptitle('PCA and t-SNE Projections \u2014 ImageNet-100 (ViT-S/16)', fontsize=16, y=1.01)
    plt.tight_layout()
...
def plot_noise_ablation(all_ablation, output_dir):
    ...
    ax1.axhline(y=1.0, color='red', linestyle=':', linewidth=1.5, alpha=0.7, label='Chance (1.0%)')
...
def plot_probe_analysis(all_probe, output_dir):
    ...
    ax.set_title('Probe analysis \u2014 ImageNet-100 (ViT-S/16)')
...
def main():
    ...
    config = {
        ...
        'protocol_reference': 'ImageNet-100 ViT-B protocol; architecture-only scaling',
        'img_size': 224,
        'patch_size': 16,
    ...
    seeds = [args.seed] if args.seed else [42, 123, 456, 789, 1011, 1213]
    ...
    # Save shared config atomically; concurrent seed sessions write identical content.
    config_path = os.path.join(args.output_dir, 'config.json')
    config_tmp = f"{config_path}.{os.getpid()}.tmp"
    ...
    if args.mode in ('train', 'all'):
        ...
        for pe_type in pe_types:
            for seed in seeds:
                ...
                run_config.update({'pe_type': pe_type, 'seed': seed})
