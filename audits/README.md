...
- `TSPG_PUBLIC_RELEASE_FINAL_FREEZE_REPORT_v1_0_20260830.json`, the machine-readable fresh-clone final content-freeze report for content commit `535eb0e7efc6e983b042d63fa420859a3391e618`;
- `TSPG_PUBLIC_RELEASE_FINAL_FREEZE_VERIFICATION_v1_0_20260830.md`, which independently rehashes/validates the supplied tree manifest and freeze report, confirms the public `main` ref remained on the frozen content commit before promotion, and closes the final content-tree SHA-256 manifest gate;
- `TSPG_PUBLICATION_AND_GITHUB_RELEASE_VERIFICATION_v1_0_20260831.md`, which records the published `v1.0.0` GitHub release, the corrected H1-0016 release-asset incident, GitHub `6/6` exact size/SHA-256 verification, Zenodo publication, and final coordinated release closeout;
- `TSPG_ZENODO_POSTPUBLICATION_REMOTE_SHA256_VERIFICATION_v1_0_20260831.md`, which closes the final archive-integrity gate by independently downloading all 14 public Zenodo objects and confirming exact byte counts, `14/14` locked SHA-256 matches, `14/14` Zenodo-MD5 matches, and aggregate downloaded size `3,535,908,885` bytes.

Machine-readable static-QA and gate outputs are promoted under `../results/`. Authoritative compact runtime evidence archives, clean-execution dependency-capture identities, the final public content-tree manifest, and current release state are recorded under `../manifests/`; large raw arrays remain outside Git and are referenced by exact hashes rather than duplicated here.
